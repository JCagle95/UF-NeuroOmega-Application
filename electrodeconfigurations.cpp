    ...
    ui(new Ui::ElectrodeConfigurations)
{
    ...
    QSettings applicationConfiguration(QDir::currentPath() + "/defaultSettings.ini", QSettings::IniFormat);
    if (!applicationConfiguration.value("DefaultChannelConfiguration").isNull())
    {
        loadConfigurations(applicationConfiguration.value("DefaultChannelConfiguration").toString());
    }
}
...
void ElectrodeConfigurations::loadConfigurations(QString fullPath)
{
    JSONStorage jsonStorage("", fullPath);
